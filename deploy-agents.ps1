...
$image = "$acr.azurecr.io/agents-backend:v1"
az acr build --registry $acr --image "agents-backend:v1" ./backend 2>&1 | Out-Null
if ($LASTEXITCODE -ne 0 -and -not (az acr repository show-tags -n $acr --repository agents-backend --query "contains(@,'v1')" -o tsv 2>$null)) {
...
$envArgs = @("PROJECT_ENDPOINT=$ep", "MODEL_DEPLOYMENT_NAME=gpt-4.1-mini", "SEARCH_MODEL_DEPLOYMENT_NAME=gpt-4.1-mini", "SEARCH_CONNECTION_NAME=$conn", "SEARCH_INDEX_NAME=enterprise-kb")
